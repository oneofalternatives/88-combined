...
against itself. Reads the `--src` dir (`attempts/final-NN`) only, like the
builder. Shapes come from the frontmatter, station columns from `SHAPES` in
extract.py, imported rather than restated — see `spec/book-format.md`. `--limit 0` for all of it, `--rule N` for
